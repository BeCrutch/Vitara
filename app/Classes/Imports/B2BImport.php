...
class B2BImport
{
    ...
    public function generateSlugProducts()
    {
        LaravelLocalization::setLocale('ru');
        $products = Product::joinLocalization()->withCategory()->get();

        foreach ($products as $product) {
            $slugName = $product->name;

            /**
             * If exist category we create Slug: $lastCategoryNameRu/$productNameRu
             */
            if ($product->category) {
                $slugName = "{$product->category->name}/{$slugName}";
            }

            $slug = Slug::create(Product::class, $slugName);
    ...
    protected function product($brandId, $productRef)
    {
        ...
        if (!isset($product)) {
            ...
            $nameRu = $productData['name'];
            $nameUk = $productData['name_uk'];
            $name = trim($productData['description']);
            $categoryName = (self::$data['categories'][$productData['category_ref']]['description'] ?? 'uncategorized');
            $slug = Slug::create($categoryName . '/' . $nameRu);//(Str::slug($categoryName) . '/' . Str::slug($name));
    ...
    public function disabledProductWithoutImage()
    {
        $products = Product::get();

        foreach ($products as $product) {
            $isExistImage = ProductImage::where('details->product_sku', $product->sku)->first();

            if (!$isExistImage || !$isExistImage->getDetails('main')) {
                Product::where('id', $product->id)->update(['details->published' => 0]);
            }
        }
    }
}
